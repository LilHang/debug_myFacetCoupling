...
int main(int argc, char** argv)
{
    ...
    auto matrixFvGridGeometry = makeBulkFVGridGeometry<MatrixFVGridGeometry>(gridManager, matrixGridView, lowDimGridView);
    ...
    // lihang add start
    // 0 refers to the matrix grid id
    auto matrixGridData = gridManager.getGridData()->template getSubDomainGridData<0>();
    auto matrixSpatialParams = std::make_shared<typename MatrixProblem::SpatialParams>(matrixFvGridGeometry,
                                                                                       matrixGridData, "Matrix");
    auto matrixProblem = std::make_shared<MatrixProblem>(matrixFvGridGeometry, matrixSpatialParams,
                                                         couplingManager, "Matrix");

    // 1 refers to the lowdim grid id
    auto lowdimGridData = gridManager.getGridData()->template getSubDomainGridData<1>();
    auto lowDimSpatialParams = std::make_shared<typename LowDimProblem::SpatialParams>(lowDimFvGridGeometry,
                                                                                       lowdimGridData, "LowDim");
    auto lowDimProblem = std::make_shared<LowDimProblem>(lowDimFvGridGeometry, lowDimSpatialParams,
                                                         couplingManager, "LowDim");
    // lihang add end
    ...
    // lihang add start
    //const double rate = matrixProblem->computeInterfaceFlux(x[matrixId], *matrixGridVariables, matrixId,*assembler, true);
    //const double rate = matrixProblem->computeOutFlow(x[matrixId], *matrixGridVariables, matrixId,*assembler, true);
    // lihang add end
